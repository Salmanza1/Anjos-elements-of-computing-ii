...
def battle_round(character, target):
    ...
    if isinstance(character, Archer):  # If the character is an Archer
        print(character.fire_arrow(target))  # Fire an arrow at the target
    elif isinstance(character, Rogue):  # If the character is a Rogue
...
if __name__ == "__main__":
    ...
    brave = Archer("Brave", 3, 80, 15, 2)
    Bezerk = Warrior("Bezerk", 5, 120, 10)

    # Simulate battle rounds
    battle_round(brave, Bezerk)  # Archer attacks Warrior
    battle_round(brave, Bezerk)     # Archer attacks Warrior
    battle_round(brave, Bezerk)  # Archer attacks Warrior again